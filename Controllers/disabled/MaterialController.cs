    ...
    public IActionResult UploadMaterial([FromBody] Material request)
    {
        ...
        try
        {
            if (string.IsNullOrEmpty(request.FileData))
                return BadRequest("Hiányzik a fájl adata (Base64 formátumban).");
            ...
            if (string.IsNullOrEmpty(request.FileName))
                return BadRequest("A fájl neve hiányzik.");
            string filePath = Path.Combine(uploadsFolder, request.FileName);
            System.IO.File.WriteAllBytes(filePath, fileBytes);

            string sql =
                "INSERT INTO Material (FileName, FileData, Subject, UploadDate, SubjectID, TeacherID) VALUES (@FileName, @FileData, @Subject, @UploadDate, @SubjectID, @TeacherID)";
            using (var connection = DatabaseConnector.CreateNewConnection())
            {
                using (var cmd = new SQLiteCommand(sql, connection))
                {
