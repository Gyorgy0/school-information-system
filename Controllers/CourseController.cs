...
public class CourseController : Controller
{
    ...
    )
    {
        using var conn = DatabaseConnector.CreateNewConnection();
        using var cmd = new SQLiteCommand(
            "INSERT INTO Course (Name, TeacherID, Visible) VALUES (@Name,@TeacherID,@Visible)",
            conn
        );
    ...
    {
        using var conn = DatabaseConnector.CreateNewConnection();
        using var cmd = new SQLiteCommand("DELETE FROM Course WHERE CourseID=@ID", conn);
    ...
    public IActionResult GetCourseEntries(int courseId)
    {
        ...
        using var conn = DatabaseConnector.CreateNewConnection();
        using var cmd = new SQLiteCommand(
            "SELECT Content FROM CourseEntries WHERE CourseID=@ID",
            conn
        );
    ...
    [HttpPost]
    public IActionResult AddCourseEntry([FromForm] int courseId, [FromForm] string content)
    {
        using var conn = DatabaseConnector.CreateNewConnection();
        using var cmd = new SQLiteCommand(
            "INSERT INTO CourseEntries (CourseID,Content) VALUES(@ID,@Content)",
            conn
        );
        cmd.Parameters.AddWithValue("@ID", courseId);
