...
public class UserController : Controller
{
    ...
    public IActionResult CheckSession()
    {
        ...
        if (string.IsNullOrEmpty(sessionId))
        {
            return Json(new { userID = -1, role = (string?)null });
        }
        ...
        // Kikeresni a role-t az adott userID alapján
        string? role = null;
        using (var connection = DatabaseConnector.CreateNewConnection())
        {
            string selectRoleSql = "SELECT Role FROM User WHERE UserID = @UserID";
            using (var cmd = new SQLiteCommand(selectRoleSql, connection))
            {
